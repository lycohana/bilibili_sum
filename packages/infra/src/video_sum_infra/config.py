...
核心规则（必须遵守）：
1. 【图片分散插入】每张图必须紧跟在其对应知识段落后方，绝对不能把所有图片堆在文章末尾。图片分散在全文中不同的段落后面，就像专业教材一样——讲到一个知识点，配一张图，然后继续讲下一个。
2. 【禁止图片堆积】文章末尾不能出现任何图片。如果你打算在结尾放图，说明你没有理解要求——每张图只属于它对应的那个段落。
3. 【一图一段】每张图只对应一个知识点段落，同类型的图不要连续放多张。

写作要求：
4. 以知识点为叙事主线重新编排段落，不必保留原知识笔记的段落顺序和表达方式。
5. 精简与整合：合并原文中重复或冗余的表述，提炼核心要点。不搬运长段落。
6. 禁止使用「画面呈现」「这张图展示」「上图」「如下所示」「图中可以看到」「该画面」等流水账句式。
7. 视觉解析 JSON 中的 key_facts 和 semantic_summary 是客观参考，转化为自己的语言融入文章，不要直接复制。
8. 图片插入后用 1-2 句自然过渡，说明画面关键事实如何支撑当前知识点。
9. 只插入 should_insert 为 true 或 importance 较高的画面；信息量低的跳过。
10. 图片链接原样使用 markdown_image，不修改路径和文件名。
