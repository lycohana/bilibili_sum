...
DEFAULT_VISUAL_NOTE_SYSTEM_PROMPT = (
    "你是一名擅长整理图文学习笔记的中文编辑。"
    "你会基于原始知识笔记和已经由视觉模型解析过的关键画面，把图片自然插入到最相关的段落后面。"
    "必须忠实原笔记和画面解析，不要编造画面外信息。只输出 Markdown 正文。"
)

DEFAULT_VISUAL_NOTE_USER_PROMPT_TEMPLATE = """请生成一版图文增强笔记。

要求：
1. 保留原知识笔记的主要结构和表达，不要改写成全新的文章。
2. 把少量关键画面自然插入到最相关的段落后面，不要堆到文末。
3. 每张图片后写 1 到 2 句画面理解，解释它为什么支撑前后的知识点。
4. 只插入 should_insert 为 true 或 importance 较高的画面；若画面信息量低，可以不插。
5. 图片链接必须原样使用 observations 中的 markdown_image，不要改路径、不要改文件名。
6. 不要使用 localhost、file:// 或绝对路径。
7. 整篇笔记仍以文字阅读为主，图片要克制。
...
DEFAULT_VISUAL_VLM_PROMPT = """请分析这张视频关键帧，并判断它是否适合插入图文笔记。

要求：
1. 用中文描述画面中的可见信息，重点关注文字、图表、公式、代码、界面、流程、演示动作和结论。
2. 判断画面与上下文知识点的关系，说明它支撑了什么内容。
3. 若画面模糊、重复、信息量低或只是人物/过场，应降低 importance 并将 should_insert 设为 false。
4. 不要编造画面外信息，不要推测看不见的内容。
5. 只输出 JSON，不要输出解释说明。

输出格式示例：
{{"description":"","key_text":[],"relevance":"","importance":1,"should_insert":false,"caption":""}}
